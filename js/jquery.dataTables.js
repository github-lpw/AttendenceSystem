        ...
        ( /** @lends <global> */ function($) {
            ...
            function _fnFilter(settings, input, force, regex, smart, caseInsensitive) {
                ...
                if (input.length <= 0) {
                    ...
                    if (reg.test(input) || reg2.test(input) && !reg3.test(input)) {
                        for (i = display.length - 1; i >= 0; i--) {
                            if (!reg2.test(input) && !rpSearch.test(settings.aoData[display[i]]._sFilterRow)) {
                                display.splice(i, 1);
                            }
                        }
                        ...
                        for (i = display.length - 1; i >= 0; i--) {
                            var reg3 = new RegExp('^\d{4}(\-|\/|.)\d{1,2}(\-|\/|.)\d{1,2}', 'g');
                            var str = settings.aoData[display[i]]._sFilterRow;
                            str = str.trim();
                            var temArray = str.split(" ");
                            var timeArray = new Array;
                            for (var j = 0; j <= temArray.length - 1; j++) {
                                if (temArray[j] != "") {
                                    timeArray.push(temArray[j]);
                                }
                            }
                            if (!(input >= timeArray[5] && input >= timeArray[7] && input <= timeArray[9])) {
                                display.splice(i, 1);
                            }
                        }
                    } else {
